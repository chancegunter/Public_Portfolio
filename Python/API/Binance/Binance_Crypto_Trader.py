...
# creating a method that appends the year_list class portion and appends csv files       
    def file_manager(self):
        if os.path.exists(r'local_path/raw.' + trader.name + '.csv') :
            f = open(r'local_path/raw.' + trader.name + '.csv','a')
            CryptoPrice = client.get_symbol_ticker(symbol= trader.binancesymbol)
            f.write(CryptoPrice['price'] + '\n')
            file_name = r'local_path/raw.' + trader.name + '.csv'
            file = open(file_name)
            ...
            f = open(r'local_path/raw.' + trader.name + '.csv','w')
            CryptoPrice = client.get_symbol_ticker(symbol= trader.binancesymbol)
            f.write(CryptoPrice['price'] + '\n')
            file_name = (r'local_path/raw.' + trader.name + '.csv')
            file = open(file_name)
...
if __name__ == "__main__":
    ...
    json.dump(results_short, open(r'local_path/results.json', 'w'))
    json.dump(results_sma, open(r'local_path/results.json', 'a'))
